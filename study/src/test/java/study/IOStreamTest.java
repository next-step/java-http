 ...
 * <p>
 * Stream은 데이터를 바이트(byte) 단위로 읽고 쓴다. 바이트가 아닌 텍스트(문자열)를 읽고 쓰려면 Reader와 Writer 클래스를 연결한다. Reader,
 * Writer는 다양한 문자 인코딩(e.g. UTF-8)을 처리할 수 있다.
 */
...
class IOStreamTest {
  ...
  class OutputStream_학습_테스트 {
    ...
    /**
     * OutputStream은 다른 매체에 바이트로 데이터를 쓸 때 사용한다. OutputStream의 서브 클래스(subclass)는 특정 매체에 데이터를 쓰기 위해
     * write(int b) 메서드를 사용한다. 예를 들어, FilterOutputStream은 파일로 데이터를 쓸 때, 또는 DataOutputStream은 자바의
     * primitive type data를 다른 매체로 데이터를 쓸 때 사용한다.
     * <p>
    ...
    void OutputStream은_데이터를_바이트로_처리한다() throws IOException {
       ...
       */
      outputStream.write(bytes); // outStream 내부 버퍼에 bytes 데이터를 담는다
      final String actual = outputStream.toString();

      assertThat(actual).isEqualTo("nextstep");
      outputStream.close(); //사용 후 close 시킨다!
    }

    /**
     * 효율적인 전송을 위해 스트림에서 버퍼(buffer)를 사용 할 수 있다. BufferedOutputStream 필터를 연결하면 버퍼링이 가능하다.
     * <p>
     * 버퍼링을 사용하면 OutputStream을 사용할 때 flush를 사용하자. flush() 메서드는 버퍼가 아직 가득 차지 않은 상황에서 강제로 버퍼의 내용을
     * 전송한다. Stream은 동기(synchronous)로 동작하기 때문에 버퍼가 찰 때까지 기다리면 데드락(deadlock) 상태가 되기 때문에 flush로 해제해야
     * 한다.
     */
    @Test
    void BufferedOutputStream을_사용하면_버퍼링이_가능하다() throws IOException {
       ...
       *
       * BufferedOutputStream은 버퍼링을 사용하여 성능을 향상 시킬 수 있다.
       * flush 를 사용하여 버퍼에 있는 내용을 강제로 출력한다.
       */
      final byte[] bytes = {110, 101, 120, 116, 115, 116, 101, 112};
  ...
  class InputStream_학습_테스트 {
    ...
    /**
     * read() 메서드는 매체로부터 단일 바이트를 읽는데, 0부터 255 사이의 값을 int 타입으로 반환한다. int 값을 byte 타입으로 변환하면 -128부터 127
     * 사이의 값으로 변환된다. 그리고 Stream 끝에 도달하면 -1을 반환한다.
     */
    @Test
  ...
  class FilterStream_학습_테스트 {
    ...
    /**
     * BufferedInputStream은 데이터 처리 속도를 높이기 위해 데이터를 버퍼에 저장한다. InputStream 객체를 생성하고 필터 생성자에 전달하면 필터에
     * 연결된다. 버퍼 크기를 지정하지 않으면 버퍼의 기본 사이즈는 얼마일까?
     * <p>
     * private static int DEFAULT_BUFFER_SIZE = 8192; 8KB 기본 사이즈를 갖고있다!
     */
    @Test
  ...
  /**
   * 자바의 기본 문자열은 UTF-16 유니코드 인코딩을 사용한다. 문자열이 아닌 바이트 단위로 처리하려니 불편하다. 그리고 바이트를 문자(char)로 처리하려면 인코딩을 신경
   * 써야 한다. reader, writer를 사용하면 입출력 스트림을 바이트가 아닌 문자 단위로 데이터를 처리하게 된다. 그리고 InputStreamReader를 사용하면
   * 지정된 인코딩에 따라 유니코드 문자로 변환할 수 있다.
   */
  @Nested
